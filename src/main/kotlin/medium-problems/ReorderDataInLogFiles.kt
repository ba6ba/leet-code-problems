...
class ReorderDataInLogFiles {
    ...
    fun reorderLogFiles(logs: Array<String>): Array<String> {
        ...
        val letterLogs = mutableListOf<LogData>()
        for (log in logs) {
            val splitIndex = log.indexOf(' ')
            if (log[splitIndex + 1].isDigit()) {
                digitLogs.add(log)
            } else {
                letterLogs.add(LogData(log.substring(0, splitIndex), log.substring(splitIndex)))
            }
        }
        letterLogs.sortWith(compareBy({ it.content }, { it.identifier }))

        val wordLogs = Array(letterLogs.size) {
            """${letterLogs[it].identifier}${letterLogs[it].content}"""
        }
        return wordLogs + digitLogs
    }
